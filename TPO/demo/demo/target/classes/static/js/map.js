...
let poiLayer = null;
let deliveryLayer = null;  // Nueva capa para delivery
...
// === FUNCIONES PARA DELIVERY MODULE ===

let deliveryFeatures = [];

export function addMarker(cornerId, label, color = '#FF0000') {
  // Esta función agrega un marcador visual en el mapa
  // Por ahora, como no tenemos las coordenadas del corner, 
  // necesitamos buscarlas via API o tenerlas en caché
  // Implementación simplificada: guardar para dibujar después
  deliveryFeatures.push({ type: 'marker', cornerId, label, color });
}

export function addPolyline(cornerIds, color = '#FF0000', width = 3) {
  // Guardar la polilínea para dibujar después
  deliveryFeatures.push({ type: 'polyline', cornerIds, color, width });
}
...
// Renderizar las features de delivery (llamar después de agregar markers/polylines)
export async function renderDeliveryFeatures() {
  if (!deliveryLayer) {
    deliveryLayer = new ol.layer.Vector({
      source: new ol.source.Vector({
        features: []
      })
    });
  ...
  const source = deliveryLayer.getSource();
  source.clear();
  
  // Por cada feature, crear el elemento OpenLayers correspondiente
  for (const item of deliveryFeatures) {
    if (item.type === 'marker') {
      // Aquí necesitaríamos las coordenadas del corner
      // Por simplicidad, asumimos que tenemos acceso a ellas
      // En una implementación real, necesitarías un caché de corners
    } else if (item.type === 'polyline') {
      // Similar para polylines
    }
  }
}
